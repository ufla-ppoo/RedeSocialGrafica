...
public class TelaRedeSocial {
    ...
    private void curtirMensagem() {
        try {
            String idMensagem = JOptionPane.showInputDialog("Id da mensagem");
            if (!idMensagem.equals("0")) {
                feed.curtir(Integer.parseInt(idMensagem));
                atualizarAreaTextoFeed();
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "O id da mensagem deve ser um número inteiro válido.");
        }
    }
    ...
    private void atualizarAreaTextoFeed() {
        ...
        // Percorre a lista de publicações adicionando na área de texto o texto da
        // publicação
